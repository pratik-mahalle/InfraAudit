...
import { AlertCircle, Check, CheckCircle2, Clock3, History, Loader2, Mail, MessageSquare, RefreshCw, Send, Siren, Webhook } from "lucide-react";
...
export function CostMonitorIncidents() {
  ...
  const escalateIncident = (incident: CostMonitorIncident) => escalate.mutate({
    ...
    onSuccess: () => toast({ title: "Incident escalated", description: "Enabled Slack, email, and webhook channels were notified again; results are recorded in history." }),
    onError: actionError,
  });
      ...
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
        ...
        {historyQuery.isLoading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin" /></div> : historyQuery.isError ? <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>History unavailable</AlertTitle><AlertDescription>{historyQuery.error instanceof Error ? historyQuery.error.message : "Try again."}</AlertDescription></Alert> : history ? (
          <div className="space-y-6">
            ...
            <section><h3 className="mb-3 flex items-center gap-2 text-sm font-semibold"><Send className="h-4 w-4" />Delivery attempts</h3><div className="space-y-2">{history.deliveries.map((delivery) => <div key={delivery.id} className={cn("rounded-lg border p-3", delivery.status === "failed" && "border-red-300 bg-red-50/50 dark:border-red-900 dark:bg-red-950/20")}><div className="flex flex-wrap items-center gap-2">{delivery.channel === "slack" ? <MessageSquare className="h-4 w-4" /> : delivery.channel === "email" ? <Mail className="h-4 w-4" /> : <Webhook className="h-4 w-4" />}<span className="font-medium capitalize">{delivery.channel}</span><Badge variant={delivery.status === "failed" ? "destructive" : "outline"}>{delivery.status}</Badge><Badge variant="secondary">attempt {delivery.attemptNumber || 1}/{delivery.maxAttempts || 1}</Badge>{delivery.retryable && delivery.status === "failed" && <Badge variant="outline">retryable</Badge>}<span className="ml-auto text-xs text-muted-foreground">{format(parseISO(delivery.attemptedAt), "PPp")}</span></div><p className="mt-2 text-xs text-muted-foreground">{delivery.trigger} · {delivery.destinationHint || "configured destination"}</p>{delivery.errorMessage && <p className="mt-2 text-sm text-red-700 dark:text-red-300">{delivery.errorCategory}: {delivery.errorMessage}</p>}</div>)}{history.deliveries.length === 0 && <p className="text-sm text-muted-foreground">No Slack, email, or webhook attempts were recorded for this incident.</p>}</div></section>
          </div>
        ) : null}
      </DialogContent>
    </Dialog>
