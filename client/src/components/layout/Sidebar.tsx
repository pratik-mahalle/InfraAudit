...
export function Sidebar({ isCollapsed = false, onToggle }: SidebarProps) {
    ...
    if (isCollapsed) {
        return (
            <TooltipProvider delayDuration={0}>
                <aside className="fixed left-0 top-14 bottom-0 w-14 bg-white dark:bg-slate-950 border-r border-gray-200 dark:border-gray-800 flex flex-col z-40 overscroll-y-contain">
                    ...
                    <nav className="flex-1 py-2 overflow-y-auto overscroll-y-contain">
                        {navGroups.map((group) => {
    ...
    return (
        <aside className="fixed left-0 top-14 bottom-0 w-56 bg-white dark:bg-slate-950 border-r border-gray-200 dark:border-gray-800 flex flex-col z-40 overscroll-y-contain">
            ...
            <nav className="flex-1 py-2 px-2 overflow-y-auto overscroll-y-contain">
                {navGroups.map((group) => {
                    const visibleItems = filterByPermission(group.items);
                    if (visibleItems.length === 0) return null;
                    return (
