                  ...
                  {isLoadingKeys ? (
                    ...
                    apiKeys.map(k => (
                      <div key={k.id} className="p-4 space-y-2">
                        {/* Newly created key — show full key with prominent copy */}
                        {k.rawKey && (
                          <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 space-y-2">
                            <div className="text-xs font-medium text-green-600 dark:text-green-400">Copy this key now — you won't see it again!</div>
                            <div className="flex items-center gap-2">
                              <code className="flex-1 text-sm font-mono bg-background px-3 py-2 rounded border truncate select-all">{k.rawKey}</code>
                              <Button variant="outline" size="sm" className="shrink-0" onClick={() => copyToClipboard(k.rawKey!, k.id)}>
                                {copiedKeyId === k.id ? <><Check className="h-3.5 w-3.5 mr-1.5 text-green-500" /> Copied!</> : <><Copy className="h-3.5 w-3.5 mr-1.5" /> Copy</>}
                              </Button>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                <code className="font-mono bg-muted px-1.5 py-0.5 rounded">{k.keyPrefix}</code>
                                <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => copyToClipboard(k.keyPrefix, k.id)}>
                                  {copiedKeyId === k.id ? <Check className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3" />}
                                </Button>
                                <span>·</span>
                                <span>Created {k.created}</span>
                                {k.expiresAt && <><span>·</span><span>Expires {k.expiresAt}</span></>}
                              </div>
                            </div>
                          </div>
