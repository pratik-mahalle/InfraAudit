...
- **Known legacy behavior:** the frontend and `POST /api/v1/providers/aws/connect` still require an AWS access key and secret key; connection validation uses static credentials; auto-sync after connect is launched from the API process; the sidebar hides the provider-status page from viewers even though backend reads are allowed; disconnect deletes current provider resources; some complete evidence and target telemetry remain unimplemented.
